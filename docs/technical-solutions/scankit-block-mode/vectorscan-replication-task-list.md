...
| K-15 | LBR 复杂断言与 NG 闭环 | N-09 | K-10、G-02 | 进行中 | lookaround、边界、状态压缩、失败回退和结果传播完整 |
| K-16 | NFA 全部 engine cost model 收敛 | N-10 | K-09~K-15 | 进行中 | 优先级、代价计算、bailout 分支和选择记录与源码行为一致 |
| K-17 | DFA/RDFA 状态传播与压缩布局对齐 | D-01~D-03 | K-07、K-16 | 进行中 | 确定化、报告/SOM/EOD、最小化、稠密/稀疏压缩和反向确认一致 |
...
| K-21 | Block 运行时源码级错误与资源闭环 | X-01、O-03 | I-01~I-07、K-17、K-19 | 进行中 | Scratch 生命周期、错误码、取消/停止、资源行为和 Block 结果语义一致 |
| K-22 | Compiler 优化与 engine cost 全量映射 | O-01、O-02 | K-08、K-16、K-17 | 进行中 | pass、触发条件、代价模型和回退原因逐项有代码证据 |
...
K-19 本轮证据：`internal/rose/rose.go` 为 Miracle 多角色候选增加首字节掩码和向量分块筛选，并提供角色状态切片复用入口；`scanner.go` 在编译期缓存不可变 Rose 角色图，并通过并发安全的状态缓冲池复用命中切片，避免每个 Block 重建角色、指令和自动机；新增 `roseSinglePool` 复用 SingleMatch 规则的去重表；无确认纯文字角色在整块扫描时直接消费已排序角色命中，跳过调度队列和重复确认。`rose.Program` 增加私有 `findRole` 与公开 `RoleByID` 入口，只读路径返回共享底层字面量的角色；`scanner.scanRoseDirectInto` 改用 `RoleByID`，消除每次候选的 Literal 防御性复制。`rose.FindMatchesInto` 增加 `matchBuf` 复用 matcher 命中切片，`findMiracleMulti` 拆分为 `findMiracleMultiInto` 复用调用方缓冲。`matchRuleInto` 和 `Engine.MatchAtInto` 在主选择路径复用结束偏移缓冲；`compiledRule` 预先计算 `containsAny`、`requiresEndOfData`、`backendEligible`、`nonGreedy`、`hasBackref` 与 `hasConditional`，避免每个起点重复遍历 AST。`fastLiteral` 路径使用扩容后的栈上候选缓冲复用 `literalFindInto` 的结果，匹配器中间 Match 缓冲进入 `sync.Pool`。`rose.MatchAt` 大小写敏感且全 ASCII 时改用 `bytes.Equal`。`ScanRuleScales` 100 规则场景分配从 13050 降至 236，1000 规则场景从 130072 降至 2053。验证命令：`go test ./internal/rose ./...`、`go test -run '^$' -bench 'ScanRuleScales' -benchtime=3x .`、`go test -race ./...`。
...
K-22 本轮证据：`compile.go` 对图优化不收敛采用原图安全回退并记录 `optimization-fallback`，并将后端资源/布局失败产生的最终 bailout 元数据写回 `compiledRule.info`，保证诊断与实际执行一致。验证命令：`go test ./...`。
...
K-23 本轮证据：执行固定规则规模、NFA 布局和 SIMD 基准，记录 `ns/op`、`B/op`、`allocs/op`、状态/边/布局内存；当前仅形成基线，不设置绝对性能阈值。Rose 计划缓存、纯文字直接候选路径及状态缓冲池已移除调度队列重复构建；NFA 专用 Spans 复用结束偏移缓冲并跳过重复布局校验，Castle/Gough 预算路径也复用结束偏移；最新 `ScanRuleScales` 在 1000 规则样本约 5.37ms、约 21.7MB 分配，后续以该基线跟踪回归。验证命令：`go test -run '^$' -bench 'ScanRuleScales|ResourceUsage|BackendEqualByteMask' -benchtime=1x ./...`。
...
- K 阶段 25 项均为源码级收敛任务；K-01～K-13 已有代码证据，K-14～K-25 仍按本轮证据持续收敛。因此“任务已补齐”仅表示清单覆盖完整，不代表源码级对齐已经完成。
